...
OUTPUT_DIR = f"{BASE_FOLDER}output_KG_story_{STORY_ID}_test"
...
for obj_scene, rel_scene in zip(object_data, relation_data):
    ...
    for triple in rel_scene["triples"]:
        ...
        nodes.update([s, t])
    # === Optional Patch: Add missing edge BEFORE drawing ===
    if "elara discovers the ancient map" in title.lower():
        source_node = merge_node("elara discovers the ancient map", nodes)
        target_node = merge_node("forest canopy", nodes)
        if not G.has_edge(source_node, target_node):
            G.add_edge(source_node, target_node, label="has_environment_object")
            print(f"[DEBUG] Patched edge added to scene: '{title}'")
    ...
    nx.draw_networkx_edge_labels(G, pos, edge_labels={(u, v): d['label'] for u, v, d in G.edges(data=True)}, font_color="red", font_size=15)
...
nx.draw(merged_graph, pos, with_labels=True, node_color="lightgreen", node_size=1800, font_size=15)
